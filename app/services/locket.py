...
def resolve_uid(username):
    ...
    try:
        ...
        def extract(text):
            ...
            if lp:
                try:
                    d = requests.utils.unquote(lp.group(1))
        ...
        return extract(res.url) or extract(html)
...
def inject_gold(uid, token_config, log_callback=None):
    def log(msg):
        if log_callback:
            log_callback(msg)
    ...
    for attempt in range(5):
        try:
            ...
            res = requests.post(url, headers=current_headers, json=body, timeout=15)
            ...
            if res.status_code == 200:
                ...
                msg = "Unknown Error"
                try:
                    msg = res.json().get('message', str(res.status_code))
                except:
